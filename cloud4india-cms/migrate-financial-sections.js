...
async function migrateFinancialServicesSections() {
  return new Promise((resolve, reject) => {
    ...
    db.get("SELECT id FROM marketplaces WHERE name LIKE '%Financial%'", (err, marketplace) => {
      if (err) {
      ...
      if (!marketplace) {
        ...
        db.all("SELECT id, name FROM marketplaces", (err, marketplaces) => {
          if (!err && marketplaces) {
            marketplaces.forEach(s => console.log(`   - ID: ${s.id}, Name: "${s.name}"`));
          }
      ...
      db.get("SELECT COUNT(*) as count FROM marketplace_sections WHERE marketplace_id = ?", [marketplace.id], (err, result) => {
        ...
        if (result.count > 0) {
          console.log(`⚠️  Financial Services already has ${result.count} sections. Skipping migration.`);
          console.log('💡 To re-run migration, delete existing sections first.');
          db.close();
          resolve();
          return;
        }
          ...
          db.run(
            ...
            function(err) {
              ...
              if (completed === total) {
                ...
                console.log('💡 Now refresh your admin panel and check the Page Sections tab.');
                db.close((err) => {
                  if (err) {
