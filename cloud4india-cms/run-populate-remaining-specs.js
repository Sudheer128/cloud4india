...
const sqlFile = path.join(__dirname, 'populate-tech-specs-remaining-products.sql');

console.log('🚀 Starting Technical Specifications Population for Remaining Products...\n');
    ...
    `, [], (err, rows) => {
      if (err) {
        ...
        rows.forEach(row => {
          ...
          console.log(`    Specifications: ${row.specification_count}`);
          console.log('');
        });
        console.log('─────────────────────────────────────────\n');
      }
      ...
      db.close((err) => {
        ...
        if (errorCount > 0) {
          ...
          console.log('\n✅ Technical Specifications population completed successfully!');
          process.exit(0);
        }
      });
    });
