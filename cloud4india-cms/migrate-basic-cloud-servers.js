...
// Use product ID 1 for Basic Cloud Servers (first product)
const productId = 1;
...
db.get("SELECT id, name FROM products WHERE id = ?", [productId], (err, product) => {
  ...
  console.log(`✅ Found product: "${product.name}" with ID: ${productId}\n`);
  
  // Check if sections already exist to prevent duplicates
  db.get("SELECT COUNT(*) as count FROM product_sections WHERE product_id = ?", [productId], (err, result) => {
    if (err) {
    ...
    if (result.count > 0) {
      console.log(`⚠️  Product already has ${result.count} sections. Skipping migration to prevent duplicates.`);
      console.log('💡 To re-run migration, first delete existing sections for this product.');
      db.close();
      return;
    }
    
    console.log('✅ No existing sections found. Proceeding with migration...\n');
    insertSections(productId);
  });
});

function insertSections(productId) {
