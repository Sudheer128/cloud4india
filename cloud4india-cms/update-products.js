...
const realProducts = [
  {
    ...
    route: '/products/basic-cloud-servers',
    order_index: 0
  },
  {
    id: 2,
    ...
    route: '/products/cpu-intensive-servers',
    order_index: 1
  },
  {
    id: 3,
    ...
    route: '/products/memory-intensive-servers',
    order_index: 2
  },
  {
    id: 4,
    ...
    route: '/products/acronis-cloud-backup',
    order_index: 3
  }
];
...
db.run('DELETE FROM products', (err) => {
  ...
  const stmt = db.prepare('INSERT INTO products (id, name, description, category, color, border_color, route, order_index, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)');
  
  realProducts.forEach((product, index) => {
    stmt.run(product.id, product.name, product.description, product.category, product.color, product.border_color, product.route, product.order_index, (err) => {
      if (err) {
