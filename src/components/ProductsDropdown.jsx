...
import { getProducts, getProductCategories } from '../services/cmsApi'
...
const ProductsDropdown = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fallback: if categories API fails, extract from products
        try {
  ...
  const productsData = products.reduce((acc, product) => {
    ...
    acc[category].push({
      ...
      buttonText: 'Explore',
      route: product.route,
      enable_single_page: product.enable_single_page,
  ...
  // Get available categories from localStorage (categories that exist but have no products yet)
  const getAvailableCategories = () => {
    try {
      const stored = localStorage.getItem('availableProductCategories');
  ...
  // Set active category to first category with products when data loads
  useEffect(() => {
  ...
  const staticProductsData = {
    'Frameworks': [
      ...
      },
      {
        id: 'lamp',
        name: 'LAMP',
        description: 'Quickly deploy a cloud server with Apache, MySQL, and PHP pre-installed—the foundation for hosting robust web applications. The LAMP stack is widely used for hosting websites and web apps, offering a flexible, scalable, and reliable App.',
        buttonText: 'Deploy'
      },
      {
        id: 'lemp',
    ...
    'Monitoring Applications': [
      {
        id: 'prometheus',
        name: 'Prometheus',
        description: 'Open source metrics and monitoring for your systems and services Monitor your applications, systems, and services with the leading open source monitoring App. Instrument, collect, store, and query your metrics for alerting, dashboarding, and other use cases',
        buttonText: 'Deploy'
      },
      {
        id: 'activemq',
  ...
  const displayProducts = activeCategory ? (productsData[activeCategory] || []) : []
      ...
      >
        <div className="px-4 sm:px-6 md:px-8 py-4 md:py-6 flex flex-col flex-1 min-h-0 overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-4 flex-shrink-0 pb-4 border-b border-gray-200">
            <h3 className="text-lg sm:text-xl font-semibold text-gray-900">Explore Products</h3>
            <button
              onClick={onClose}
            ...
            <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
              <div className="mb-3 md:mb-4 flex-shrink-0">
                <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">
                  {displayCategories.find(c => c.id === activeCategory)?.label || 'Products'}
                </h2>
                      ...
                      const cardContent = (
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-gray-600 text-xs sm:text-sm line-clamp-2 sm:line-clamp-3 leading-relaxed">{product.description}</p>
                          </div>
