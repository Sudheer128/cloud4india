...
const ProductBasicInfo = ({ product, onSave, saving }) => {
  ...
  useEffect(() => {
    if (product && product.id !== 'new') {
      // Clean route - remove /products/ prefix if present
      let cleanRoute = product.route || '';
      if (cleanRoute.startsWith('/products/')) {
        cleanRoute = cleanRoute.replace('/products/', '');
      } else if (cleanRoute.startsWith('products/')) {
        cleanRoute = cleanRoute.replace('products/', '');
      }
      ...
      setFormData({
        ...
        route: cleanRoute,
        icon: product.icon || '',
        enable_single_page: product.enable_single_page !== undefined ? product.enable_single_page : 1,
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-600 mt-1">Essential details about your product</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g., GPU Compute, Object Storage, Load Balancer"
          />
          {errors.name && <p className="mt-1 text-sm text-red-600">{errors.name}</p>}
          <p className="mt-1 text-xs text-gray-500">This will be the main heading on your product page</p>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Brief description of your product and its main benefits"
          />
          {errors.description && <p className="mt-1 text-sm text-red-600">{errors.description}</p>}
          <p className="mt-1 text-xs text-gray-500">This appears on the products listing page and in search results</p>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                When enabled, clicking this product opens its dedicated detail page
              </p>
            </div>
            <button
              type="button"
          ...
          {!formData.enable_single_page && (
            <div className="mt-4 p-4 bg-white rounded-lg border border-blue-200">
              ...
              />
              <p className="mt-1 text-xs text-gray-500">
                When users click this product, they will be redirected to this URL instead of the detail page
              </p>
            </div>
          )}
        </div>
          ...
          >
            {saving ? (
              ...
              <>
                ...
                {product && product.id !== 'new' ? 'Update Product' : 'Create Product'}
              </>
            )}
          </button>
        </div>
