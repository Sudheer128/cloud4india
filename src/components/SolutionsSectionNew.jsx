...
import { useMainSolutionsContent, useHomepageContent } from '../hooks/useCMS'
...
const SolutionsSectionNew = () => {
  ...
  const config = homepageData?.sectionsConfig?.solutions || {}
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        setCategories(['all', 'Industry', 'Use Case'])
      }
    }
    fetchCategories()
  }, [])
    ...
    <section className="py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-4xl font-light text-gray-900">{config.heading || 'Explore our Solutions'}</h2>
        </div>
        ...
        {solutions.length > 6 && (
          <div className="text-center mt-12">
            ...
            >
              {config.button_text || 'View more Solutions'}
            </button>
          </div>
        )}
      </div>
