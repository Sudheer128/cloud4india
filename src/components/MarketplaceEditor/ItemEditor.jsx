...
const ItemEditor = ({ item, sectionType, sectionId, marketplaceId, onSave, onCancel }) => {
  ...
  useEffect(() => {
    if (item) {
      ...
      if (item.features) {
        try {
          parsedContent.features = JSON.parse(item.features);
        } catch (e) {
    ...
    try {
      ...
      if (['pricing', 'specifications', 'use_cases', 'media_banner'].includes(sectionType)) {
        ...
        if (sectionType === 'media_banner') {
          ...
          if (!contentJSON.media_source && contentJSON.media_type === 'video') {
      ...
      console.log('Payload content type:', typeof payload.content);
      console.log('Payload content length:', payload.content ? payload.content.length : 'null/undefined');
  ...
  // Handle file upload for media items
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                console.log('YouTube URL changed to:', newUrl);
                setContentJSON(prev => {
                  const updated = { ...prev, media_url: newUrl };
                  console.log('Updated contentJSON:', updated);
                  return updated;
                });
              }}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg"
            ...
            {contentJSON.media_url && (
              <p className="text-xs text-green-600 mt-1">✓ URL entered: {contentJSON.media_url}</p>
            )}
          </div>
        ) : (
          <div>
  ...
  const renderGenericForm = () => {
    const needsDescription = ['feature', 'benefit', 'security_feature', 'support_channel', 'step', 'segment', 'technology', 'ai_ml_section', 'roi_stat', 'metric', 'resource', 'featured_resource', 'implementation', 'timeline_phase', 'use_case_card'].includes(formData.item_type);
    const needsIcon = ['feature', 'benefit', 'security_feature', 'support_channel', 'segment', 'technology', 'ai_ml_section', 'resource', 'featured_resource'].includes(formData.item_type);
    ...
    return (
      <>
        ...
        {/* Features field for benefit, segment, and technology items */}
        {['benefit', 'segment', 'technology', 'ai_ml_section', 'use_case_card', 'featured_resource'].includes(formData.item_type) && (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Key Features (one per line)
              {formData.item_type === 'benefit' && ' (optional)'}
            </label>
            <textarea
              value={(contentJSON.features || []).join('\n')}
              onChange={(e) => setContentJSON(prev => ({ ...prev, features: e.target.value.split('\n').filter(Boolean) }))}
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              Optional checklist items shown below the description
            </p>
          </div>
        )}

        {/* Value and Label fields */}
        {['segment', 'resource', 'featured_resource', 'implementation', 'timeline_phase', 'ai_ml_section'].includes(formData.item_type) && (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
        ...
        {/* Value field for ROI stat items */}
        {['roi_stat', 'metric'].includes(formData.item_type) && (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Stat Value *</label>
            <input
              type="text"
              ...
              onChange={(e) => setFormData(prev => ({ ...prev, value: e.target.value }))}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg"
              placeholder="e.g., 40%, 3x, 60%"
              required
            />
            <p className="text-xs text-gray-500 mt-1">The large stat value to display</p>
          </div>
        )}
