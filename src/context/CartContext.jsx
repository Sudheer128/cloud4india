...
const ACTIONS = {
  ADD_ITEM: 'ADD_ITEM', REMOVE_ITEM: 'REMOVE_ITEM', UPDATE_ITEM: 'UPDATE_ITEM',
  UPDATE_QUANTITY: 'UPDATE_QUANTITY', UPDATE_DURATION: 'UPDATE_DURATION',
...
export function CartProvider({ children }) {
  ...
  const subtotalByCategory = state.items.reduce((acc, item) => { acc[item.item_type] = (acc[item.item_type] || 0) + (parsePrice(item.unit_price) * item.quantity); return acc; }, {});
  ...
  return (
    <CartContext.Provider value={{ items: state.items, isCartOpen: state.isCartOpen, addItem, removeItem, updateItem, updateQuantity, updateDuration, clearCart, toggleCart, itemCount, subtotal, subtotalByCategory, itemsByCategory, parsePrice }}>
      {children}
