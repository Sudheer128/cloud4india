...
const SectionEditor = ({ section, sectionTypes, onCreate, onUpdate, onCancel }) => {
  ...
  useEffect(() => {
    if (section) {
      ...
      // Parse detailed content if it exists
      try {
        if (section.detailed_content) {
          setDetailedData(JSON.parse(section.detailed_content));
        } else {
          setDetailedData({});
        }
      } catch (error) {
        console.error('Error parsing detailed content:', error);
        setDetailedData({});
      }
    } else {
      setFormData({
  ...
  const handleSubmit = (e) => {
    ...
    // Update formData with current detailed content
    const updatedFormData = {
      ...formData,
      detailed_content: JSON.stringify(detailedData)
    };
  ...
  const updateDetailedData = (key, value) => {
  ...
  const updateNestedDetailedData = (path, value) => {
    setDetailedData(prev => {
      const newData = { ...prev };
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="border-b border-gray-200">
            <nav className="-mb-px flex space-x-8">
              ...
              <button
                type="button"
                onClick={() => setActiveDetailTab('detailed')}
                className={`py-2 px-1 border-b-2 font-medium text-sm ${
          ...
          {activeDetailTab === 'detailed' && (
            <div className="space-y-6">
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                <h6 className="text-sm font-medium text-blue-900 mb-2">🎯 Detailed Content Editor</h6>
                <p className="text-xs text-blue-800">
                  This section will allow editing of all detailed content like bullet points, statistics, features, and other structured data for each section type.
                </p>
              </div>
              
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
                <h6 className="text-sm font-medium text-yellow-900 mb-2">🚧 Coming Soon</h6>
                <p className="text-xs text-yellow-800">
                  The detailed content editor is being implemented. For now, you can edit the basic title and content in the "Basic Info" tab.
                </p>
              </div>
            </div>
          )}
