...
import DynamicSolutionSection from '../components/DynamicSolutionSection';
...
const UniversalSolutionPage = () => {
  ...
  // Determine if solutionName is a numeric ID or a route slug
  // If it's a number, use it as-is. If it's a string (route), pass it directly.
  const isNumericId = !isNaN(parseInt(solutionName)) && solutionName.toString().match(/^\d+$/);
  const solutionIdentifier = isNumericId ? parseInt(solutionName) : solutionName;
  
  const { sections, itemsBySection, solution, loading, error } = useSolutionData(solutionIdentifier);
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-4">Error Loading Solution</h2>
  ...
  if (!sections || sections.length === 0) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-4">Solution Not Found</h2>
          <p className="text-sm text-gray-600 mb-4">This solution doesn't exist or has no content.</p>
          <button 
