...
const FinancialServices = () => {
  ...
  // Helper function to get section data by order
  const getSectionByOrder = (order) => {
    return sections.find(section => section.order_index === order);
  };

  // Helper function to get section items
  const getSectionItems = (sectionId) => {
    // This will be handled by individual sections that need items
    return [];
  };

  // Component for dynamic benefit cards
  const DynamicBenefitCards = ({ sectionId }) => {
    const { items, loading, error } = useSectionItems(1, sectionId);
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {items.map((item, index) => {
          ...
          return (
            <div key={item.id} className={`bg-white rounded-2xl p-8 shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-200 ${borderClass}`}>
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {items.map((item, index) => {
          ...
          const IconComponent = iconMap[item.icon] || RocketLaunchIcon;
          
          return (
            <div key={item.id} className={`bg-white rounded-2xl p-8 text-center shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-200 ${borderClass}`}>
