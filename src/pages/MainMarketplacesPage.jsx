...
const MainMarketplacesPage = () => {
  ...
  }, [marketplaces, searchTerm, selectedCategory]);

  // Get visible marketplaces based on pagination
  const visibleMarketplaces = filteredMarketplaces.slice(0, visibleCount);
  ...
  const hasMore = visibleCount < filteredMarketplaces.length;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-saree-teal-light via-white to-saree-lime-light flex items-center justify-center">
        <div className="text-center">
          <LoadingSpinner />
          <p className="text-gray-700 mt-4">Loading our innovative marketplaces...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-saree-teal-light via-white to-saree-lime-light flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-4">Error Loading Marketplaces</h2>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            ...
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Search marketplaces & services"
                value={searchTerm}
          ...
          {/* Marketplace Count Display */}
          <div className="mb-5">
            <p className="text-xs text-gray-600">
              Displaying {filteredMarketplaces.length > 0 ? 1 : 0}-{Math.min(visibleCount, filteredMarketplaces.length)} ({filteredMarketplaces.length})
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-6">
            {visibleMarketplaces && visibleMarketplaces.length > 0 ? (
              visibleMarketplaces.map((marketplace, index) => {
                // Map marketplace to section format for MarketplaceCard
                // Use section data if available, otherwise fall back to marketplace data
                const section = {
                  id: marketplace.id,
                  marketplace_id: marketplace.marketplace_id || marketplace.id,
                  title: marketplace.title || marketplace.name,
                  description: marketplace.description,
                  category: marketplace.category,
                  popular_tag: marketplace.popular_tag || 'Most Popular',
                ...
                return (
                  <MarketplaceCard 
                    ...
                    section={section} 
                    index={index}
                    isHovered={hoveredCard === marketplace.id}
...
// Modern Marketplace Card Component - Optimized & Compact
const MarketplaceCard = ({ section, index, isHovered, onHover }) => {
  // Icon mapping for different marketplace types
  const getMarketplaceIcon = (title) => {
  ...
  const IconComponent = getMarketplaceIcon(section.title);
...
export default MainMarketplacesPage;