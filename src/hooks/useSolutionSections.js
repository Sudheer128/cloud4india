...
export const useSolutionSections = (solutionId) => {
  ...
  const fetchData = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const response = await fetch(`${import.meta.env.VITE_CMS_URL || (import.meta.env.PROD ? 'http://161.97.155.89:4002' : 'http://localhost:4002')}/api/solutions/${solutionId}/sections`);
